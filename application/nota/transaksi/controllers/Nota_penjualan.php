...
class Nota_penjualan extends CI_Controller {
	...
	function simpan_dt_penjualan()
	{
		$d = array(
					...
					if($bayar==0 && $jenis_bayar=="BON" )
					{
						$q = "insert into tbl_penjualan_header(kd_transaksi,kd_pelanggan,total,bayar,status,ket_transaksi)
									values('".$kode_trans."','".$kd_pelanggan."','".$total."','0','0','".$ket_transaksi."')";
						$this->Penjualan->simpan_penjualan($q);
						foreach($this->cart->contents() as $items)
						{
							$this->Penjualan->simpan_penjualan("insert into tbl_penjualan_detail (kd_transaksi,kd_brg,nm_brg,harga,jumlah,status) values('".$kode_trans."','".$items['id']."','".$items['name']."','".$items['price']."','".$items['qty']."','0')");
	...
	function cetak_kasbon()
	{
		...
		if(count($d['detail']->result())>0)
		{
	        ...
	        $this->load->view('transaksi/nota_penjualan/cetak_nota/v_bon',$d);
		}
		else {
			echo 'Tidak Ada Data';
		}
